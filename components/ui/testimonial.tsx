...
// Generar avatar placeholder basado en iniciales
function generateAvatarPlaceholder(name: string): string {
  ...
  return `data:image/svg+xml,${encodeURIComponent(`
    <svg width="48" height="48" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect width="48" height="48" fill="url(#grad)" rx="24"/>
      <text x="50%" y="50%" font-family="Arial, sans-serif" font-size="18" font-weight="bold" fill="white" text-anchor="middle" dominant-baseline="central">${initials}</text>
    </svg>
  `)}`
}
...
export function Testimonial({ reviews }: TestimonialProps) {
  const [activeIndex, setActiveIndex] = useState(0)
  const [isHovered, setIsHovered] = useState(false)
  const containerRef = useRef<HTMLDivElement>(null)

  // Convertir reviews a formato testimonial
  const testimonials = reviews.map((review) => {
    const authorDisplay = review.isAnonymous
      ? "Anónimo"
      : review.authorPillName || review.authorName || "Anónimo"
    
    const role = "Paciente"
    const company = "Psicología Clínica"
    ...
    return {
      ...
      role,
      company,
      avatar: generateAvatarPlaceholder(authorDisplay),
      reviewId: review.id,
